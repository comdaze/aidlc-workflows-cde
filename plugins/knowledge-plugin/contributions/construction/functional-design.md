...
- any **gate rejection reason** from a prior revision loop of this stage
  ("this is wrong, it should be X" → candidate `[correction]`/`[pitfall]`);
- design decisions made against the domain ("we chose A because…" →
  `[decision]`);
- domain facts discovered that the knowledge base lacks or contradicts
  (→ `[pitfall]` with anchor).
