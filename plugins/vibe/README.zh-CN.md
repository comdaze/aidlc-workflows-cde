...
插件带了两个文件，让它从"被调用"变成"被选中"：

- `agents/aidlc-vibe.md` —— 人格文件。它既是 stage 的 `lead_agent`，**也是**选择器条目的 prompt，
  所以两个入口的行为完全一致。
- `agents/aidlc-vibe.json` —— Kiro agent 配置。`resources` 在会话开始时就把记忆层、这个席位的知识、
  以及两个 runner skill 钉进上下文，也就是说**读取端由 agent 自己保证**，不再依赖常开的 steering 文件。

**两个文件刻意共用同一个文件名词干。** Kiro 把 `.kiro/agents/` 下的 `.md` 和 `.json` 都当 agent
配置读，所以两个不同的词干会生成**两个**选择器条目 —— 而 `.md` 那个不带 `resources`、`tools`、
`toolsSettings`，等于更显眼的那个恰好是功能退化的那个。修掉之前在真实安装里实测到：被选中的是人格文件那个，
于是整个会话跑在没有工具面、记忆也没被钉入的状态下。共用词干是核心那 14 个 agent 的做法，
它把这一对收拢成一个名字。**不要只改其中一个的名字。**
