...
Sections 1–5 take you to a first install; **§6 is the loop you repeat after that**,
and it exists because three of its steps fail silently — no error, no drop record,
exit 0 — so they are worth reading before you assume an edit took effect.
...
## 6. Iterating on a plugin you have already installed

Everything above gets you to a first install. The *second* edit onwards has three
steps that fail **silently** if you skip them — no error, no drop record, exit 0.
Each one below is a measured failure, not a caution.

**The loop:**

```bash
# 1. regenerate the projections — dist/ is generated, and it is what everything reads
bun scripts/package.ts

# 2. remove the installed copies — compose will NOT overwrite them (see below)
cd <project>
rm -f <harness-dir>/agents/<your-agent>.md \
      <harness-dir>/scopes/<your-scope>.md \
      <harness-dir>/aidlc-common/stages/<phase>/<your-stage>.md
rm -rf <harness-dir>/knowledge/<your-agent>

# 3. re-compose, then verify
AIDLC_PLUGIN_ROOT=<repo>/dist/plugins/<name>/<harness> AIDLC_PROJECT_DIR=<project> \
  AIDLC_HARNESS_DIR=<harness-dir> bun "$AIDLC_PLUGIN_ROOT/hooks/compose.ts"
bun <harness-dir>/tools/aidlc-utility.ts doctor
```

### Compose is no-clobber, and that is invisible

Compose never overwrites a file the project already has. Re-composing after an
edit therefore **keeps the old bytes, prints nothing, and exits 0** — the install
silently stays one version behind the source you just edited.

The trap is what it does *not* produce: **a no-clobber skip writes no `.drops`
record.** Drops exist for contributions the composer had to abandon (missing
target, malformed anchor, a key the installed engine rejects), not for "the file
was already there". So `doctor` looks clean and the hook-health files look clean
while your change is not installed. Deleting the installed copies first, as in
step 2, is the only reliable refresh.

> [!WARNING]
> Before deleting, check whether the harness dir is **git-ignored** in the target
> project (dogfooding installs commonly are, via `.git/info/exclude`). If it is,
> git cannot restore those files — copy them to a temp dir first. This is easy to
> get wrong in the other direction too: because the paths are ignored, the install
> may be invisible in your editor's file tree while being present on disk.

### `dist/` is what runs, and what the tests read

The authored source is `core/` + `plugins/<name>/`; `dist/<harness>/` and
`dist/plugins/<name>/<harness>/` are generated by `bun scripts/package.ts` and
committed. Two consequences:

- **The test suites read `dist/`, not `core/`.** A test cannot be exercised by
  editing `core/` alone — without repackaging first, a green result is
  indistinguishable from a real pass. If you are unsure whether a guard actually
  covers your change, remove the fix, repackage, and confirm the guard goes red.
- **`bun scripts/package.ts --check` is the drift gate.** It fails when `dist/`
  disagrees with the source, which is also how a "source edited, dist not
  regenerated" commit gets caught. Run it before you commit.

### Verify with `doctor`, and read the stage count

`doctor` is the cheap check that composition landed *and* that the plugin is
enabled:

```
✓  Enabled plugins: aidlc, <name>; enabled stage counts: aidlc=30, bootstrap=3, <name>=1
✓  Hook drops: none recorded
```

A stage count of `0` for your plugin means the files are installed but the stage
is filtered out of the graph — the selection is absolute, not additive, so
`select-plugins` must name **every** plugin you want enabled:

```bash
bun <harness-dir>/tools/aidlc-utility.ts select-plugins aidlc,<name>
```

That state — installed but not selected — is worth recognising, because a scope
runner or agent entry will still be present and will open a container whose stage
does not exist.

### Contributions need their own check

A green test suite does not tell you a contribution still applies. Verify it
against the **installed** target stage, and check the frontmatter rather than
grepping for a marker: `adds:` entries are merged into the target's YAML lists,
not wrapped in sentinel comments, so a marker grep returns nothing and reads like
a failure when nothing is wrong.

```bash
sed -n '/^---$/,/^---$/p' <project>/<harness-dir>/aidlc-common/stages/<phase>/<target>.md \
  | grep '<plugin>-'
```

This matters most after a framework upgrade: if upstream renames a stage or moves
a fragment anchor, the merge succeeds, the suite stays green, and the contribution
**stops applying silently**. Re-run this check after any upgrade, not only after
your own edits.

### Version the plugin, not the framework

A plugin carries its own version in `.aidlc-plugin/plugin.json`. Bump it when
plugin behaviour changes; leave the framework's version and changelog alone.
