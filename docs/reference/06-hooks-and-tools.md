...
| `rebuild-stage-graph.ts` | PostToolUse | Project-wide (settings.json) | `Bash` | Recompile `runtime-graph.json` on transition-class audit emits |
...
3. **Health heartbeat:** Writes `.aidlc-hooks-health/rebuild-stage-graph.last`.
...
1. **bun** -- Required for all 16 hooks and every CLI tool (`aidlc-utility.ts`, `aidlc-state.ts`, `aidlc-jump.ts`, `aidlc-orchestrate.ts`, `aidlc-audit.ts`, `aidlc-validate.ts`, `aidlc-graph.ts`, `aidlc-sensor.ts`, `aidlc-learnings.ts`, `aidlc-runtime.ts`). Install via `brew install bun` or `npm install -g bun`. On Windows: `npm install -g bun`. Must be on PATH for non-interactive shells.
