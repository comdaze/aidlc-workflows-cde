...
**Worked example — test-pro across a mixed fleet.** A platform team publishes `test-pro` once (author against `core/`, `bun scripts/package.ts`, push a `<plugin>--v<version>` tag, drop a `marketplace.json`). Claude teams `/plugin install`; Codex teams `codex plugin add` (approve trust once); Kiro teams `git pull` + run the composer explicitly (above). In every case the composer merges test-pro's two new stages **and** its contributions to `build-and-test`/`nfr-requirements`/`nfr-design`/`performance-validation` — the same enriched, 34-stage, doctor-clean install. Validated across all five harness projections (Claude, Codex, Kiro CLI, Kiro IDE, opencode) — measured as identical per-harness compose output: every plugin stage, scope, sensor, stage runner + scope runner on disk, the orchestrator's scope/stage tables refreshed, and zero compose drops.

Two host-shape traps that used to make that parity silently untrue, both fixed and worth knowing if you port to a sixth harness:

- **Where the runner skills live.** Compose probes `<harness-dir>/skills` to decide whether to regenerate runners, but Codex discovers skills at `<project>/.agents/skills/` and ships nothing under `.codex/skills/`. The naive probe missed it, so a Codex install got every plugin file *except* an entry point — no `/<plugin-scope>` runner — with only an advisory drop to show for it. The probe now mirrors `resolveSkillsPath`'s `.codex` fallback, which is what `aidlc-runner-gen` writes through; if those two ever disagree, compose decides against one directory while the generator writes another.
- **The table markers are exact strings.** The scope-grid and stage-graph regions are spliced by literal `<!-- BEGIN: … -->` / `<!-- END: … -->` sentinels shared by compose and `aidlc-utility.ts scope-table` / `stage-table`. opencode's authored `SKILL.md` had an em dash where the literal has a hyphen, and no stage-graph pair at all — so **both** table refreshes skipped, for plugins *and* for the framework's own regeneration. A one-character drift in a sentinel is a silently dead code path; the harness-porting checklist ([Porting to a New Harness](../harness-engineering/09-porting-to-a-new-harness.md)) is where to verify it.
