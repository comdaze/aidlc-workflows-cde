...
its command filter, `sync-statusline` deriving stage from the audit tail because
"the IDE gives no task payload", and the reviewer-scope note that Kiro IDE "ships
no registration … a pre-tool matcher has nothing to inspect there".

**Not verified, and it gates that work:** `tool_input` for *write* tools
(`fs_write` / `str_replace` / `fs_append`). The probe's `PostToolUse` matcher was
`execute_bash`, so only shell payloads were captured. Measure this first — the
prose-scraping removal depends entirely on it.

Also noted: `cwd` is the hook-owning workspace, and `file_path` may point
**outside** it (observed: a `cwd` in one workspace with a `file_path` in
another). Do not assume `file_path` is under `cwd`.
...
4. **Hooks are picked up in the current session.** The `createHook` tool reports
   "active on the next session start"; measured, the control fired on the very
   next tool call. Do not restart on that advice alone.
...
- `aidlc-block` is a `preToolUse` hook with **no tool filter**, and `preToolUse`
  exit 2 *is* a working block (the adapter calls it a live-verified contract). So
  while an AI-DLC gate is open with no human turn since, it blocks the tool calls
  a spec task needs — and Kiro shows no reason why.
