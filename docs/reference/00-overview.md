...
| [Hooks and Tools](06-hooks-and-tools.md) | Hook system, CLI tools, 74-event audit taxonomy |
...
| [State Machine](12-state-machine.md) | Workflow / phase / stage machines, 74-event taxonomy, audit-first rules |
...
| [Kiro Spec Integration Boundary](19-kiro-spec-integration.md) | Measured limits of merging with Kiro's native Spec mode: the spec task triggers and their payloads, why `PreTaskExec` confers no veto, which integration shapes are viable, and the richer v2 hook channel the harness does not yet target |
| [Fork Divergence](20-fork-divergence.md) | Every deviation from upstream `aidlc-workflows` outside `plugins/`, why it exists, which rows are upstream-bound, the conflict-resolution recipe per logical change, and the upstream sync procedure including the compose-drop check |
