...
> [!IMPORTANT] The framework is a hard prerequisite, and getting the order wrong fails **silently**. The composer returns immediately — no error, no health record — when `<project>/<harness-dir>/tools/aidlc-graph.ts` is missing, because it validates every plugin stage against the *installed* engine's schema and agent roster before copying anything. Compose is idempotent, so a mis-ordered attempt only needs a re-run once the framework is in place.
