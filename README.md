...
![version](https://img.shields.io/badge/version-2.6.61-blue)
...
- **[86-event audit trail](docs/guide/10-state-and-audit.md)** - structured logging for enterprise traceability
...
| Hooks die with `/bin/sh: bun: command not found` (127) even though `~/.zshenv` exports it | Kiro IDE | That harness spawns hooks through `/bin/sh`, which reads **no** rc file — neither `~/.zshrc` nor `~/.zshenv` (zsh-only) — so it sees only the PATH its parent handed down. Launched from the GUI, that parent is launchd, whose default PATH excludes `~/.bun/bin`. Put bun on a system path instead: `ln -s "$HOME/.bun/bin/bun" /usr/local/bin/bun`. Launching the IDE from a terminal also works, but only for that session. `/aidlc --doctor` checks this directly. A 127 leaves no drop record — the hook never ran — so the hook-health files look clean while nothing fires. |
