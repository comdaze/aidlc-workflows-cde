...
- **bun**: Required for the CLI tools and hook scripts (state management, audit logging, orchestration engine). Install via \`brew install bun\` or \`npm install -g bun\`, or with bun's own installer \`curl -fsSL https://bun.sh/install | bash\`. \`bun\` must be on your PATH for the shells opencode spawns; the AIDLC adapter plugin also probes \`~/.bun/bin/bun\` directly.
