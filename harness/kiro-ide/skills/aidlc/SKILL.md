...
Each `next` reads the workflow state and the compiled stage graph and returns **exactly one** typed directive (JSON) on stdout. It mutates nothing. The directive's `kind` names the single move to make; you make that move, then `report` commits the resulting transition so the next `next` reads fresh state. **Report each lifecycle outcome once; never call lifecycle verbs on `aidlc-state.ts` directly** — a gated directive reports `awaiting-approval`, then any `rejected`/`revised` cycles, then `approved`; the engine dispatches every state transition, and a speculative direct call gets the state-guard error from the state CLI itself (Kiro IDE registers no state-transition PreToolUse hook — tool-argument delivery is not uniform across supported builds, so the CLI's ownership check is the enforcement here, same pattern as the prose-only reviewer-scope bound; code-generation's plan-approval guard is likewise prose-only on this harness — never dispatch the developer agent before the human's explicit "Approve Plan"). Pass `$ARGUMENTS` through to the first `next` verbatim — the engine parses flags (`--status`, `--stage`, `--scope`, `--depth`, freeform text, …) and resolves the scope, so you do not pre-parse or strip them.
