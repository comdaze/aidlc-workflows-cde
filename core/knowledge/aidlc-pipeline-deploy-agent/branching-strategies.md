...
**Worktree mapping.** Create: `bun {{HARNESS_DIR}}/tools/aidlc-worktree.ts create --slug <bolt-slug> --base main`. Merge: `--target main --strategy squash`. Each Bolt = one squash commit on `main`.
...
5. **Merge**: caller must be on `<integration>` at the main checkout. `bun {{HARNESS_DIR}}/tools/aidlc-worktree.ts merge --slug <bolt-slug> --target <integration> --strategy <merge|squash>`.
