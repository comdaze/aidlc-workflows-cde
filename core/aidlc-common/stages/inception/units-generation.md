...
Load aidlc-delivery-agent persona from `agents/aidlc-delivery-agent.md` and knowledge from `{{HARNESS_DIR}}/knowledge/aidlc-delivery-agent/` for feasibility validation and prioritization.
...
`bun {{HARNESS_DIR}}/tools/aidlc-orchestrate.ts report --stage units-generation --result <outcome>`.
