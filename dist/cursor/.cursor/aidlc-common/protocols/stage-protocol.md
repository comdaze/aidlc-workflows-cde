...
2. [ ] **Log non-gate questions via `aidlc-log.ts`** — before presenting a structured question that is not an approval gate: `bun .cursor/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .cursor/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. Approval choices go only through `aidlc-orchestrate.ts report`. (§2, §3)
...
`.cursor/aidlc-common/protocols/stage-protocol-construction.md`.
...
.cursor/tools/aidlc-utility.ts scope-table` when you need those
...
- **Consolidated summary before generation**: After all questions have been answered, present a consolidated summary of all answers as unordered bullets (never a numbered list), then present this structured question:
  ...
  `bun .cursor/tools/aidlc-log.ts decision --stage <slug>
  ...
  `bun .cursor/tools/aidlc-log.ts answer --stage <slug>
  --checkpoint summary-confirmation --questions-file "<questions-path>"
...
1. BEFORE presenting the approval question: `bun .cursor/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval`.
...
1. BEFORE presenting the question: `bun .cursor/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .cursor/tools/aidlc-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
```bash
bun .cursor/tools/aidlc-orchestrate.ts report \
...
```bash
bun .cursor/tools/aidlc-state.ts lookup phase-of SLUG          # → phase name
bun .cursor/tools/aidlc-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .cursor/tools/aidlc-state.ts lookup agent-for SLUG          # → lead agent name
bun .cursor/tools/aidlc-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
2. `.cursor/knowledge/aidlc-shared/` — shared methodology principles
3. `.cursor/knowledge/[agent-name]/` — agent-specific methodology
...
`.cursor/aidlc-common/protocols/stage-protocol-ensemble.md`.
...
.cursor/tools/aidlc-utility.ts scope-table` for the current
...
| **Scope** | Controls which stages execute and at what depth. Eleven built-in scopes, one file per scope under `.cursor/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, classic, workshop, express. Custom scopes can be added without editing this file. |
...
`.cursor/aidlc-common/protocols/stage-protocol-reviewer.md`.
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .cursor/tools/aidlc-learnings.ts surface --slug <stage-slug>
   ```
...
5. **Persist (the tool writes + emits audit).** Build the selections file — `{ stage_slug, space, intent, selections[] }`, where `space`/`intent` are carried verbatim from step 2's surface output — and call:
   ```bash
   bun .cursor/tools/aidlc-learnings.ts persist --slug <stage-slug> --selections-json <path>
   ```
...
```bash
bun .cursor/tools/aidlc-state.ts reuse-artifact <stage-slug> \
