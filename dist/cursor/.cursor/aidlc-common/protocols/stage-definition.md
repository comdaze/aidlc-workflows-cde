...
under `.cursor/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.cursor/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.cursor/tools/data/stage-graph.json`. Consumers continue to read the compiled
