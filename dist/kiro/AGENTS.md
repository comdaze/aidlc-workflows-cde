...
- **Agents**: `.kiro/agents/` — 14 agents: 11 domain-expert personas (product, design, delivery, architect, aws-platform, compliance, devsecops, developer, quality, pipeline-deploy, operations), 2 review-only agents (product-lead, architecture-reviewer), and the adaptive-workflows composer. On Kiro the conductor is `agents/aidlc.json`; all 14 personas have JSON configs, and workers for the four dispatched stages (2.1 pipeline, 2.2 subagent, 2.4 mob, 3.5 subagent), reviewer passes, and composer requests run through the Kiro `subagent` tool, while inline-stage personas are adopted in-context.
