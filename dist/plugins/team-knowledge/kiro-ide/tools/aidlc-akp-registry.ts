...
export function buildRegistry(bundleRoot: string, today: Date): { entries: RegistryEntry[]; unreadable: string[] } {
  ...
  for (const abs of discoverCardPaths(bundleRoot)) {
    ...
    entries.push({
      ...
      class: str(dig("cde.class", fm)),
      generalization: str(dig("cde.generalization", fm)),
      destination: type === "Practice" ? str(dig("cde.heading", fm)) : str(dig("cde.knowledge_seat", fm)),
      origin_project: str(dig("cde.origin.project", fm)),
      supersedes: str(dig("cde.supersedes", fm)),
      path: abs,
    });
  }
