...
2. [ ] **Log non-gate questions via `aidlc-log.ts`** — before presenting a structured question that is not an approval gate: `bun .codex/tools/aidlc-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .codex/tools/aidlc-log.ts answer --stage <slug> --details "<exact choice>"`. Approval choices go only through `aidlc-orchestrate.ts report`. (§2, §3)
...
- **Consolidated summary before generation**: After all questions have been answered, present a consolidated summary of all answers in a clear list, then present this structured question:
  ...
  `bun .codex/tools/aidlc-log.ts decision --stage <slug>
  ...
  `bun .codex/tools/aidlc-log.ts answer --stage <slug>
  --checkpoint summary-confirmation --questions-file "<questions-path>"
...
1. BEFORE presenting the approval question: `bun .codex/tools/aidlc-orchestrate.ts report --stage <slug> --result awaiting-approval`.
