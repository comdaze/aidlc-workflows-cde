...
// and seeds nothing; it doubles as a forward-check that the 6 shipped manifests
...
//       rows.length === 6 (same observable: count of tab-bearing rows).
...
//
// 12 migrated cases + claim-sources + traceability describe cases -> 14 test() cases.
...
describe("t93 aidlc-sensor describe (migrated from t93-sensor-list-describe.sh, plan 12)", () => {
  ...
  test("10: describe traceability includes the canonical command and JSON match", () => {
    const r = sensor("describe", "traceability");
    expect(r.status).toBe(0);
  ...
  test("11: describe <unknown-id> exits 1 with a known-ids hint", () => {
...
describe("t93 aidlc-sensor help + dispatch (extended from t93-sensor-list-describe.sh, plan 13)", () => {
  test("12: --help prints usage covering all 3 subcommands", () => {
    const r = sensor("--help");
  ...
  test("13: -h prints the same usage banner", () => {
    const r = sensor("-h");
    expect(r.out).toContain("Usage: aidlc-sensor");
  });

  test("14: no subcommand exits 1 with a usage hint", () => {
    const r = sensor();
